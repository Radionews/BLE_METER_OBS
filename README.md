...
4. **Добавление в OBS**  
   В OBS добавьте источник «Браузер» → укажите URL `http://localhost:8080`. Рекомендуемая ширина 400, высота 100.
...
- `ACV / DCV` – переменное/постоянное напряжение
- `ACmV / DCmV` – милливольты
- `OHM` – сопротивление (Ω, кΩ, МΩ)
- `CAP` – ёмкость (нФ, мкФ, мФ)
- `Hz` – частота (Гц, кГц, МГц)
- `%` – коэффициент заполнения (duty cycle)
- `DCA / ACA` – сила тока (мкА, мА, А)
- `°C / °F` – температура
- `CONT` – прозвонка
- `DIDOE` – проверка диодов
